...
window.addEventListener("load", () => {
      ...
      for (let j = 0; j < el.length; j++) {
        ...
        try {
          let response = await fetch(
            `./sounds/c${i + 1}/${Object.keys(nt)[0]}.wav`
          );
  ...
  preloadAudio().then(() => {
    pianoData.forEach((data, i) => {
      ...
      semiNote5.setAttribute("data-key", data["cord_" + (i + 1)][10][note11].key);
      ...
      noteDiv7.setAttribute("data-key", data["cord_" + (i + 1)][11][note12].key);
  ...
  window.addEventListener("keydown", (e) => {
    if (!customizeKeyModal.classList.contains("active")) {
      ...
      const noteElement = document.querySelector(`.switch[data-key="${e.key}"]`);
